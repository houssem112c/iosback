...
router.post('/generate/:id',  async (req, res) => {
  try {
    ...
    console.log(userId);
    const token = await verificationTokenController.generateVerificationToken(userId);
   ...
   // const renderedTemplate = compiledTemplate(data);
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        ...
        res.status(200).json({ message: 'Verification token generated and sent successfully' });
      }
    });
    res.status(200).json({ message: 'Verification token generated and sent successfully' });
  } catch (error) {
...
router.post('/verify/:id/:pin', async (req, res) => {
  try {
    ...
    const isVerified = await verificationTokenController.verifyUser(userId, pin);
